...
def main():
    print_log("Starting the Data Careers application")
    ...
    article = st.selectbox("Select an article that you want to get more from:", article_list)
    print_log(f"User selected article: {article}")
    ...
    search_type = st.radio("Select search type:", ["Vector"])
    ...
    if st.button("Ask"):
        ...
        with st.spinner("Processing..."):
            ...
            answer_data = get_answer(user_input, article, model_choice, search_type)
            end_time = time.time()
            ...
            save_conversation(
                st.session_state.conversation_id, user_input, answer_data, article
            )
